...
namespace XkbPlugin
{
    ...
    public class ConfigDialog : Gtk.Dialog
    {
        ...
        private string build_options_str(GenericSet<string> options)
        {
            GLib.StringBuilder strb = new StringBuilder();
            options.foreach((s)=>{
                if (strb.len > 0)
                    strb.append(",");
                strb.append(s);
        ...
        {
            TreeIter iter;
            GenericSet<string> options = new GenericSet<string>(str_hash,str_equal);
            ...
            plugin.backend.options = build_options_str(options);
        }
        [GtkCallback]
